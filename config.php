...
function isAdmin() {
    ...
    // Check session timeout - only logout if timeout exceeded, don't auto-refresh
...
function isUser() {
    ...
    // Check session timeout - only logout if timeout exceeded, don't auto-refresh
    if (isset($_SESSION['user_last_activity']) && (time() - $_SESSION['user_last_activity'] > SESSION_TIMEOUT)) {
        logoutUser();
        return false;
    }
